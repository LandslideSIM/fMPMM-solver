...
for sim=1:length(numel)
    ...
    t       = 1.25                                                         ;% simulation time             [s]
    te      = 0.25                                                         ;% elastic loading             [s]
    %---------------------------------------------------------------------%
    
    %% MESH INITIALIZATION
    Lx    = 0.80
    [meD] = meSetupCollapse(numel(sim),Lx,typeD)                          ;% - see function   
    % BOUNDARY CONDITIONS
    yinf    = 0.0                                                         ;%
...
% %         %------------------------------------------------------------------%
        ...
        if((plasticity)&&(dt*it>te))
            [mpD,dat] = plastic(mpD,Gc,Kc)                                  ;% - see function
        end
        cycle_time(it,5)=toc                                              ;%
    ...
    fig2=figure(54363);
    clf
    set(fig2,'Units','pixels','Position',[1083 531 364 353]);
