...
mpD.U(:,1:nDoF:end) = meD.u(ix)                                           ;% nodal x-displacement
mpD.U(:,2:nDoF:end) = meD.u(iy)                                           ;% nodal y-displacement
for mp = 1:mpD.n
    iDx = nDoF*p2n(mp,:)-1                                                ;% x-component index
...
mpD.J = mpD.F(:,1).*mpD.F(:,4)-mpD.F(:,3).*mpD.F(:,2)                     ;% determinant of the incremental deformation tensor
mpD.V = mpD.J.*mpD.V0                                                     ;% volume of mp
if(uGIMP==1)
    mpD.l = mpD.l0                                                        ;% half-length of mp
elseif(cpGIMP==1)
    mpD.l = mpD.J.*mpD.l0                                                 ;% half-length of mp
    %mpD.l = mpD.dD(:,[1 4]).*mpD.l;
    %mpD.dU= sqrt([(mpD.dD(:,1).^2+mpD.dD(:,3).^2),...
    %    (mpD.dD(:,2).^2+mpD.dD(:,4).^2)])                                 ;% stretch part of incremental deformation, see Charlton etal, 2017
