...
% %         %------------------------------------------------------------------%
        ...
        %% BASIS FUNCTIONS
        tic;
        [mpD] = SdS_loop(meD,mpD,p2.N)                                         ;% - see function 
        cycle_time(it,1)=toc;
        %------------------------------------------------------------------%
        %% MAPPING FROM MATERIAL POINTS (p) TO NODES (N)
        tic
        [meD] = p2Nsolve_loop(meD,mpD,g,dt,l2g,p2.N,BC)                        ;% - see function
        cycle_time(it,2)=toc; 
        %------------------------------------------------------------------%

        %% MAPPING FROM NODES (N) TO MATERIAL POINTS (p)
        tic
        [meD,mpD] = mapN2p_loop(meD,mpD,dt,l2g,p2.N,BC)                        ;% - see function
        cycle_time(it,3)=toc;
        %------------------------------------------------------------------%
        
        %% UPDATE INCREMENTAL DEFORMATION & STRAIN 
        tic
        [mpD] = DefUpdate_loop(meD,mpD,dt,l2g,p2.N,meD.DoF)               ;% - see function
        cycle_time(it,4)=toc;
        %------------------------------------------------------------------%   
...
%     plot(1:length(runt),runt(:,1),'r--',1:length(runt),runt(:,2),'k-')
    ...
    name=['.\data\GIMP_time_iterative_' num2str(sim) '.mat'];
    save(name,'runt','cycle_time','tsolve');
    meanDPIperRun(sim) = mean(cycle_time(:,end));
    
    name=['.\data\GIMP_iterative_data_' num2str(sim) '.mat'];
