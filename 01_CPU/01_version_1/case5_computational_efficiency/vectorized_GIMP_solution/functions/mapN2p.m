...
% VELOCITY UPDATE                                                          
mpD.v(:,1) = mpD.PF(1)*(mpD.v(:,1)+dt*sum(mpD.S.*meD.a(iDx),2))+...
             mpD.PF(2)*(sum(mpD.S.*meD.v(iDx),2))                         ;% PIC-FLIP update
mpD.v(:,2) = mpD.PF(1)*(mpD.v(:,2)+dt*sum(mpD.S.*meD.a(iDy),2))+...
             mpD.PF(2)*(sum(mpD.S.*meD.v(iDy),2))                         ;% PIC-FLIP update
% MOMENTUM UPDATE
mpD.p      = mpD.v.*repmat(mpD.m,1,meD.DoF)                               ;% update mp momentum
% COORDINATE UPDATE
mpD.x      = mpD.x+dt*[sum(mpD.S.*meD.v(iDx),2) sum(mpD.S.*meD.v(iDy),2)] ;% update mp position
%--------------------------------------------------------------------------%
...
P        = reshape([mpD.S.*repmat(mpD.p(:,1),1,meD.nNe) ;...
                    mpD.S.*repmat(mpD.p(:,2),1,meD.nNe)],...
