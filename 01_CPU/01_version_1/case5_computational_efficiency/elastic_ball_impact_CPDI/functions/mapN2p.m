...
% VELOCITY UPDATE                                                          
mpD.v(:,1) =  mpD.PF(1)   .*(mpD.v(:,1)+dt.*accumarray(mp,mpD.S.*meD.a(nX),[mpD.n 1]))+...
    mpD.PF(2).*(accumarray(mp,mpD.S.*meD.v(nX),[mpD.n 1]))                      ;% PIC-FLIP update
mpD.v(:,2) = mpD.PF(1)   .*(mpD.v(:,2)+dt.*accumarray(mp,mpD.S.*meD.a(nY),[mpD.n 1]))+...
    mpD.PF(2).*(accumarray(mp,mpD.S.*meD.v(nY),[mpD.n 1]))                    ;% PIC-FLIP update
%------------------------------------------------------------------%
...
mpD.x = mpD.x+dt*[accumarray(mp,mpD.S.*meD.v(nX),[mpD.n 1]),...
                  accumarray(mp,mpD.S.*meD.v(nY),[mpD.n 1])];
%% UPDATE NODAL MOMENTUM WITH UPDATED MP MOMENTUM (MUSL OR DOUBLE MAPPING)
meD.p(:) = 0.0                                                            ;% initialized momentum
meD.p    = accumarray([nX;nY],[mpD.S.*mpD.p(mp,1);...
...
clear nX nY iD BCx BCy                                                    ;% clear temporary variables
%--------------------------------------------------------------------------%
