...
for sim=1:length(numel)
    ...
    while((tw<t)||(sum(isnan(mpD.v(:,1))+isnan(mpD.v(:,2)))>0.0))          % BEGIN WHILE LOOP
        ...
        tic
        [mpD] = DefUpdate(meD,mpD,l2g,c2N)                                  ;% - see function
        cycle_time(it,4)=toc;
        %------------------------------------------------------------------%   
  ...
  free_surf=load('C:\Users\agnes\Desktop\MPM\work\technical_note\case3_ElastoPlasticSlump\Huang_etal_Solution\free_surface.txt');
    fail_surf=load('C:\Users\agnes\Desktop\MPM\work\technical_note\case3_ElastoPlasticSlump\Huang_etal_Solution\failure_surface.txt');
