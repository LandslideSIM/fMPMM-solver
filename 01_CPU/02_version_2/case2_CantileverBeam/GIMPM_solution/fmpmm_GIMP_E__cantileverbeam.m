...
for sim=1:length(numel)
    ...
    BC      = reshape(bc.x(:,1),length(bc.x(:,1))/2,2)
    BC(:,1) = BC(:,2)/2;
%    plot(meD.x,meD.y,'s',meD.x(BC(:,1)),meD.y(BC(:,1)),'x',meD.x,meD.y,'s',meD.x(BC(:,1)-meD.nNy),meD.y(BC(:,1)-meD.nNy),'bx',mpD.x(:,1),mpD.x(:,2),'o',meD.x,meD.y,'s'),axis equal
...
subplot(122)
du = mpD.s(2,:);%sqrt(mpD.u(:,1).^2+mpD.u(:,2).^2);
