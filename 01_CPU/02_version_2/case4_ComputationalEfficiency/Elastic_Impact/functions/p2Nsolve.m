...
%                  repmat((permute(mpD.s',[3 2 1])),meD.nDoF(1),1),2).*...
%                  repmat( permute(mpD.V ,[3 2 1]) ,meD.nDoF(1),1))          ;% 
% 
% fi= squeeze(sum(mpD.B.*repmat(reshape(mpD.s,4,1,mpD.n),1,meD.nDoF(1)).*repmat((reshape(mpD.V,1,1,mpD.n)),4,meD.nDoF(1)),1)); %     
%          
%fi= squeeze(sum(mpD.B.*repmat(reshape(mpD.s,size(mpD.s,1),1,mpD.n),1,meD.nDoF(1)).*reshape(mpD.V,1,1,mpD.n),1)); %     
%fi= squeeze(sum(mpD.B.*repmat(reshape(mpD.s,size(mpD.s,1),1,mpD.n),1,meD.nDoF(1)),1).*reshape(mpD.V,1,1,mpD.n)); % 
fi= squeeze(sum(mpD.B.*repmat(reshape(mpD.s,size(mpD.s,1),1,mpD.n),1,meD.nDoF(1)),1)).*repmat(mpD.V',meD.nDoF(1),1); % 

% fi1 = fi; 
% tl = zeros(size(fi1));
% for k=1:mpD.n
%     tl(:,k) = mpD.V(k,1).*(mpD.B(:,:,k)'*mpD.s(:,k));
% end 
%  
% 
% tl-fi
