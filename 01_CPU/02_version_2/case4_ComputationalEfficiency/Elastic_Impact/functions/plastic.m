function [mpD,D] = plastic(mpD,Hp,cohr,phir,Del)
...
phi          = mpD.phi                                                    ;%
% % EXPONENTIAL
% nu           = 2.5                                                       ; % the lower the higher the decrease in strength
% coeff        = exp(-nu.*mpD.epII)                                        ;
% c            = cohr+(mpD.coh-cohr).*coeff                                ;
% phi          = phir+(mpD.phi-phir).*coeff                                ;
...
f            = tau+sig.*sin(phi)-c.*cos(phi)                      ;% yield function f<=0, if f>0 then return mapping
mpD.sn       = mpD.s                                                      ;% store stress 
%--------------------------------------------------------------------------%

%% PLASTIC CORRECTION: ONE STEP EXACT RETURN MAPPING
beta         = abs(c.*cos(phi)-sig.*sin(phi))./tau                ;%
dsigA        = 0.5*beta.*(ds)                                             ;%
dsigB        = c./tan(phi)                                            ;%
