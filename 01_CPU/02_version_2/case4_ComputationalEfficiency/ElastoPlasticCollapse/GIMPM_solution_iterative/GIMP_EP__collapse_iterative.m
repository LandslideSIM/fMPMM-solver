...
for sim=1:length(numel)
    ...
    ly      = 0.1                                                      ;% layer thickness [m]
    [mpD]   = mpSetup(meD,ni,ly,coh0,0.0,phi0,0.0,n0,rho0,nstr,typeD)   ;% - see function
%     mpD.s(2,:) = -rho0.*g.*(ly-mpD.x(:,2));
%     mpD.s(1,:) = (nu./(1-nu)).*mpD.s(2,:);
    meD.vd=0.0;
    ...
    % PLOT SETTING
    [pp] = plotSetup(meD.xB(1),meD.xB(2),0.0,ly,meD.y,bc.y)                       ;%
    %---------------------------------------------------------------------%
    ...
    while((tw<t)||(sum(isnan(mpD.v(:,1))+isnan(mpD.v(:,2)))>0.0))% BEGIN WHILE LOOP
        ...
        DT(it) = dt;
        if(tw<=(te/1.5) && te>0.0)
            g = 9.81/(te/1.5)*tw                                          ;% incremented gravity load
        else
            g = 9.81                                                      ;%
        end
        %------------------------------------------------------------------%
        ...
        p2.N  = meD.e2N(p2.e,:)                                           ;% index mp to node 
        l2g  = [meD.DoF*p2.N-1;meD.DoF*p2.N]                              ;% local to global node index list
        %------------------------------------------------------------------%
        %% BASIS FUNCTIONS
        tic;
        [mpD] = SdS_loop(meD,mpD,p2.N)                                    ;% -
        cycle_time(it,1)=toc;
        %------------------------------------------------------------------%
        %% MAPPING FROM MATERIAL POINTS (p) TO NODES (N)
        tic
        [meD] = p2Nsolve_loop(meD,mpD,g,dt,l2g,p2.N,bc)                   ;% -
        cycle_time(it,2)=toc;
        %------------------------------------------------------------------%
        %% MAPPING FROM NODES (N) TO MATERIAL POINTS (p)
        tic
        [meD,mpD] = mapN2p_loop(meD,mpD,dt,p2.N,bc)                       ;% -
        cycle_time(it,3)=toc;
        %------------------------------------------------------------------%
        %% UPDATE INCREMENTAL DEFORMATION & STRAIN RATE
        tic
        [mpD] = DefUpdate_loop(meD,mpD,dt,l2g,p2.N,meD.nDoF)              ;% -
        cycle_time(it,4)=toc;
        %------------------------------------------------------------------%   
