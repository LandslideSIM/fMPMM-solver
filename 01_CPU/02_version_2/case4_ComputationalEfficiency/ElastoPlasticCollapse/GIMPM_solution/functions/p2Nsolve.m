...
p = reshape([mpD.S.*repmat(mpD.p(:,1),1,meD.nNe);...
             mpD.S.*repmat(mpD.p(:,2),1,meD.nNe)],mpD.n*meD.nDoF(1),1)    ;% preprocesing of momentum global vector
f = reshape([mpD.S.*0.0                         ;...
             mpD.S.*repmat(mpD.m,1,meD.nNe).*-g ],mpD.n*meD.nDoF(1),1)    ;% preprocesing of external force global vector  
% fi= squeeze(sum(permute(mpD.B  ,[2 1 3]).*...
%                 repmat((permute(mpD.s',[3 2 1])),meD.nDoF(1),1),2).*...
%                 repmat( permute(mpD.V ,[3 2 1]) ,meD.nDoF(1),1))           ;% preprocesing of internal force global vector  
...
meD.mr     = reshape(repmat(meD.m',meD.DoF,1),meD.nDoF(2),1)              ;% repmat nodal mass
