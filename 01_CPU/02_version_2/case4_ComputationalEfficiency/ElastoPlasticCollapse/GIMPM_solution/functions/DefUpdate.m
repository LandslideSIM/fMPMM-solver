...
elseif(cpGIMP==1)
   mpD.l = repmat(mpD.J.^(meD.DoF),1,size(mpD.l,2)).*mpD.l0               ;% update based on the determinant of F
%     mpD.dU= sqrt([(mpD.F(:,1).^2+mpD.F(:,2).^2),...
%                   (mpD.F(:,3).^2+mpD.F(:,4).^2)])                       ;% update based on the diagonal components of U, see Charlton etal, 2017
%     mpD.l = mpD.dU.*mpD.l0                                              ;% -   
%     mpD.l = mpD.F(:,[1 4]).*mpD.l0                                        ;% update based on the diagonal components of F, see Charlton etal, 2017
end
