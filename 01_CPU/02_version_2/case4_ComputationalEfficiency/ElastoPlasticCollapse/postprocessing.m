...
for run = 1:1
GIMP         = load(['.\GIMPM_solution\data\GIMP_data_' num2str(run) '.mat']);
...
hold on
ax3=scatter(x*1e3,y*1e3,10,disp,'filled');
...
ax1=plot(data_surface(:,1)*1e3,data_surface(:,2)*1e3,'bs:','LineWidth',3);
ax2=plot(data_failure(:,1)*1e3,data_failure(:,2)*1e3,'bo:','LineWidth',3);
box on;
...
print(gcf,['figElastoPlasticCollapse_' num2str(run)],'-dpng');
% 
% 
